...
Object.extend(Object.extend(Ajax.ZkInPlaceEditor.prototype,Ajax.InPlaceEditor.prototype),{
   ...
   createForm: function() {
    ...
    if (this.options.rows == 1 && !this.hasHTMLLineBreaks(text)) {
	  ...
	  // unescapeHTML() no codifica "<",">","%", etc..
	  textArea.value = this.convertHTMLLineBreaks(text).unescapeHTML();
  ...
  onSubmit: function(ev) {
    ...
    var val = new Validation(this.form,{stopOnFirst:true});
    //Validation.validate('field_id')
	var boo =  Validation.validate(this.editField.id)
...
Object.extend(Object.extend(Ajax.ZkInPlaceAutoCompleteEditor.prototype,Ajax.InPlaceEditor.prototype),{
    initialize: function(element, url, id, autoUrl,grid, options)
    {
		this.viewPort = grid.viewPort;
 		this.refreshBuffer =  function(resp)
 					 {
  ...
  createForm: function() {
    ...
    this.element.parentNode.insertBefore(this.form, this.element);
	new Ajax.ZkAutocompleter(this.getId2Update()+"value",this.getId2Update()+"valueUpdate",this.getAutoUrl(),{inPlaceEditor:this});
...
{
 initialize: function(liveGrid, viewedRows, totalRowsId, gapId)
 {
...
Object.extend(Object.extend(Ajax.ZkAutocompleter.prototype,Ajax.Autocompleter.prototype),{
    ...
    function(element, update){
      if(!update.style.position || update.style.position=='absolute') {
        ...
        Position.clone(element, update, {setHeight: false, offsetTop: element.offsetHeight, setWidth:false});
      }
